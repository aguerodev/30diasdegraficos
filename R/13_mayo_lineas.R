...
ggplot(data = datos, mapping = aes(x = fecha)) +
  geom_line(mapping = aes(y = positivos, color = "1Total Casos"),
            size = 1) +
  geom_line(mapping = aes(y = activos, color = "2Activos"),
            size = 1) +
  geom_line(mapping = aes(y = recuperados, color = "3Recuperados"),
            size = 1) +
  scale_x_date(date_labels = "%B\n%d", date_breaks = "week") +
  labs(
    title = "Costa Rica: Casos positivos, activos y recuperados de Covid19",
    subtitle = "Datos actualizados al 12 de mayo del 2020",
    color = "",
    y = "Cantidad de casos",
    x = "",
    caption = "Fuente: http://geovision.uned.ac.cr/oges/"
  ) +
  scale_color_manual(
